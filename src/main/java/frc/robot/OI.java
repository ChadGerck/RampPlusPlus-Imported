...
public class OI {
    ...
    public double getLeftJoystickAngle() {  return Math.toDegrees(Math.atan2(xbox.getRawAxis(0), -xbox.getRawAxis(1))); }
    public double LeftMag()  { return Math.hypot(xbox.getRawAxis(1), xbox.getRawAxis(0)); }
    public double RightMag() { return Math.hypot(xbox.getRawAxis(4), xbox.getRawAxis(5)); }

    
    public double LeftTrigger() { double raw = xbox.getRawAxis(2); return Math.abs(raw) < DEADZONE_LIMIT ? 0.0 : raw; }
    public double RightTrigger() { double raw = xbox.getRawAxis(3); return Math.abs(raw) < DEADZONE_LIMIT ? 0.0 : raw; }
    ...
    public boolean DpadUp() { double POV = xbox.getPOV(); 
        if((POV >= 0 && POV < 45) || (POV >= 315 && POV < 360)) { return true; } else { return false; }
    } public boolean DpadRight(){ double POV = xbox.getPOV(); 
        if(POV >= 45 && POV < 135) { return true; } else { return false; }
    } public boolean DpadDown() { double POV = xbox.getPOV(); 
        if(POV >= 135 && POV < 225) { return true; } else { return false; }
    } public boolean DpadLeft() { double POV = xbox.getPOV(); 
